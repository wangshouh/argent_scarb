...
fn hash_domain(domain: @StarkNetDomain) -> felt252 {
    let mut state = pedersen::pedersen(0, STARKNET_DOMAIN_TYPE_HASH);
    state = pedersen::pedersen(state, *domain.name);
    state = pedersen::pedersen(state, *domain.version);
    state = pedersen::pedersen(state, *domain.chain_id);
    pedersen::pedersen(state, 4)
}
...
fn hash_outside_call(outside_call: @Call) -> felt252 {
    ...
    loop {
        match data_span.pop_front() {
            Option::Some(item) => {
                call_data_state = pedersen::pedersen(call_data_state, *item);
            },
            Option::None(_) => {
                break;
            },
    ...
    call_data_state = pedersen::pedersen(call_data_state, outside_call.calldata.len().into());

    let mut state = pedersen::pedersen(0, OUTSIDE_CALL_TYPE_HASH);
    state = pedersen::pedersen(state, (*outside_call.to).into());
    state = pedersen::pedersen(state, *outside_call.selector);
    state = pedersen::pedersen(state, outside_call.calldata.len().into());
    state = pedersen::pedersen(state, call_data_state);
    pedersen::pedersen(state, 5)
}
...
fn hash_outside_execution(outside_execution: @OutsideExecution) -> felt252 {
    ...
    let mut state = pedersen::pedersen(0, OUTSIDE_EXECUTION_TYPE_HASH);
    state = pedersen::pedersen(state, (*outside_execution.caller).into());
    state = pedersen::pedersen(state, *outside_execution.nonce);
    state = pedersen::pedersen(state, (*outside_execution.execute_after).into());
    state = pedersen::pedersen(state, (*outside_execution.execute_before).into());
    state = pedersen::pedersen(state, (*outside_execution.calls).len().into());
    state = pedersen::pedersen(state, outside_calls_state);
    pedersen::pedersen(state, 7)
}
...
fn hash_outside_execution_message(outside_execution: @OutsideExecution) -> felt252 {
    ...
    let mut state = pedersen::pedersen(0, 'StarkNet Message');
    state = pedersen::pedersen(state, hash_domain(@domain));
    state = pedersen::pedersen(state, get_contract_address().into());
    state = pedersen::pedersen(state, hash_outside_execution(outside_execution));
    pedersen::pedersen(state, 4)
}
